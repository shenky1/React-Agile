...
namespace TrelloAPI.Services
{
    public class CommentService : ICommentService
    {
        ...
        public async Task<IEnumerable<CommentResponse>> GetCommentsForCard(long id)
        {
            var comments = await _commentRepository.GetCommentsForCard(id);

            var commentResponse = MapModelToResponse(comments);
            return commentResponse;
        }
        ...
        public Comment MapRequestToModel(CommentRequest commentRequests)
        {
            ...
            var comment = new Comment
            {
                ...
                UserId = commentRequests.UserId,
                CardId = commentRequests.CardId,
                Content = commentRequests.Content,
                DateCreated = commentRequests.DateCreated
            };
